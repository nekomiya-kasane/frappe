...
struct window_view : pipeable_base<window_view<PartitionBy, OrderBy, WindowFns...>> {
    ...
    constexpr window_view(PartitionBy pb, OrderBy ob, WindowFns... wfs)
        : partition_by(std::move(pb)), order_by(std::move(ob)), 
          window_fns(std::move(wfs)...) {}
    ...
    constexpr auto operator()(R&& r) const {
        ...
        for (auto& [key, indices] : partitions) {
            ...
            for (std::size_t i = 0; i < partition_size; ++i) {
                ...
                auto window_values = std::apply([&](const auto&... fns) {
                    return std::make_tuple(fns(ctx, items[idx])...);
                }, window_fns);
...
struct window_builder {
    ...
    template<typename... WindowFns>
    [[nodiscard]] constexpr auto select(WindowFns... fns) const {
        return window_view<PartitionBy, OrderBy, WindowFns...>{
            partition_by, order_by, std::move(fns)...
        };
    }
};
...
inline constexpr struct window_fn {
    ...
    [[nodiscard]] constexpr auto operator()(OrderBy order_by) const {
        return window_builder<no_partition_fn, OrderBy>{
            no_partition_fn{}, std::move(order_by)
        };
    }
} window{};
...
inline constexpr auto row_number = [](const auto& ctx, const auto&) { 
    return ctx.row_number; 
};

inline constexpr auto rank = [](const auto& ctx, const auto&) { 
    return ctx.rank; 
};

inline constexpr auto dense_rank = [](const auto& ctx, const auto&) { 
    return ctx.dense_rank; 
};

inline constexpr auto partition_size = [](const auto& ctx, const auto&) {
    return ctx.partition_size;
};

inline constexpr auto partition_index = [](const auto& ctx, const auto&) {
    return ctx.partition_index;
};

template<typename Proj>
constexpr auto lag(Proj proj, std::size_t offset = 1) {
    return [p=std::move(proj), offset](const auto& ctx, const auto&) 
        -> std::optional<std::invoke_result_t<Proj, std::decay_t<decltype(*ctx.prev)>>> 
    {
...
constexpr auto lag(Proj proj, std::size_t offset, Default default_value) {
...
template<typename Proj>
constexpr auto lead(Proj proj, std::size_t offset = 1) {
    return [p=std::move(proj), offset](const auto& ctx, const auto&) 
        -> std::optional<std::invoke_result_t<Proj, std::decay_t<decltype(*ctx.next)>>> 
    {
...
template<typename Proj, typename Default>
constexpr auto lead(Proj proj, std::size_t offset, Default default_value) {
    return [p=std::move(proj), offset, dv=std::move(default_value)](const auto& ctx, const auto&) {
        if (ctx.partition_index + offset < ctx.partition_size && ctx.partition_data && ctx.partition_indices) {
            auto target_idx = (*ctx.partition_indices)[ctx.partition_index + offset];
            return std::invoke(p, (*ctx.partition_data)[target_idx]);
...
constexpr auto ntile(std::size_t n) {
    return [n](const auto& ctx, const auto&) {
        return (ctx.partition_index * n) / ctx.partition_size + 1;
    };
}
...
template<typename OrderBy = std::identity>
struct with_row_number_view : pipeable_base<with_row_number_view<OrderBy>> {
    OrderBy order_by;
    ...
    constexpr auto operator()(R&& r) const {
        ...
        if constexpr (!std::is_same_v<OrderBy, std::identity>) {
            std::sort(items.begin(), items.end(), [this](const T& a, const T& b) {
                return std::invoke(order_by, a) < std::invoke(order_by, b);
            });
        }
