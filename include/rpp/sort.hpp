...
struct sort_view : pipeable_base<sort_view<Proj, Comp>> {
    ...
    constexpr auto operator()(R &&r) const {
        ...
        if (descending) {
            ::ranges::sort(result,
                           [this](const T &a, const T &b) { return comp(std::invoke(proj, b), std::invoke(proj, a)); });
        } else {
            ::ranges::sort(result,
                           [this](const T &a, const T &b) { return comp(std::invoke(proj, a), std::invoke(proj, b)); });
        }
        return owning_view<std::vector<T>>{std::move(result)};
    }

    [[nodiscard]] constexpr auto desc() const { return sort_view<Proj, Comp>{proj, comp, true}; }

    [[nodiscard]] constexpr auto asc() const { return sort_view<Proj, Comp>{proj, comp, false}; }

    [[nodiscard]] constexpr auto as_comparator() const { return comparator<Proj, Comp>{proj, comp, descending}; }
};

template <typename Proj, typename Comp> sort_view(Proj, Comp, bool) -> sort_view<Proj, Comp>;

template <typename Proj> sort_view(Proj) -> sort_view<Proj, std::less<>>;
...
inline constexpr struct sort_fn {
    [[nodiscard]] constexpr auto operator()() const { return sort_view<>{}; }

    template <typename Comp> [[nodiscard]] constexpr auto operator()(Comp comp) const {
...
struct stable_sort_view : pipeable_base<stable_sort_view<Proj, Comp>> {
    ...
    template <input_range R> constexpr auto operator()(R &&r) const {
        ...
        if (descending) {
            ::ranges::stable_sort(
                result, [this](const T &a, const T &b) { return comp(std::invoke(proj, b), std::invoke(proj, a)); });
        } else {
            ::ranges::stable_sort(
                result, [this](const T &a, const T &b) { return comp(std::invoke(proj, a), std::invoke(proj, b)); });
        }
        return owning_view<std::vector<T>>{std::move(result)};
    }

    [[nodiscard]] constexpr auto desc() const { return stable_sort_view<Proj, Comp>{proj, comp, true}; }

    [[nodiscard]] constexpr auto asc() const { return stable_sort_view<Proj, Comp>{proj, comp, false}; }
};
...
inline constexpr struct stable_sort_fn {
    [[nodiscard]] constexpr auto operator()() const { return stable_sort_view<>{}; }
} stable_sort{};
...
template <typename Proj = std::identity> struct natural_sort_view : pipeable_base<natural_sort_view<Proj>> {
    Proj proj;
    bool descending = false;

    constexpr natural_sort_view(Proj p = {}, bool desc = false) : proj(std::move(p)), descending(desc) {}
    ...
    template <input_range R> constexpr auto operator()(R &&r) const {
        ...
        if (descending) {
            ::ranges::sort(result, [this](const T &a, const T &b) {
                return natural_comparator::natural_compare(std::invoke(proj, b), std::invoke(proj, a));
            });
        } else {
            ::ranges::sort(result, [this](const T &a, const T &b) {
                return natural_comparator::natural_compare(std::invoke(proj, a), std::invoke(proj, b));
            });
        }
        return owning_view<std::vector<T>>{std::move(result)};
    }

    [[nodiscard]] constexpr auto desc() const { return natural_sort_view<Proj>{proj, true}; }

    [[nodiscard]] constexpr auto asc() const { return natural_sort_view<Proj>{proj, false}; }
};

inline constexpr struct natural_sort_fn {
    [[nodiscard]] constexpr auto operator()() const { return natural_sort_view<>{}; }
...
template <typename... Projs> struct multi_sort_view : pipeable_base<multi_sort_view<Projs...>> {
    std::tuple<Projs...> projs;

    constexpr explicit multi_sort_view(Projs... ps) : projs(std::move(ps)...) {}
    ...
    template <input_range R> constexpr auto operator()(R &&r) const {
        ...
        ::ranges::sort(
            result, [this](const T &a, const T &b) { return compare_impl(a, b, std::index_sequence_for<Projs...>{}); });
  ...
  private:
    ...
    constexpr bool compare_impl(const T &a, const T &b, std::index_sequence<Is...>) const {
        bool result = false;
        bool decided = false;
        ((decided || (invoke_compare(std::get<Is>(projs), a, b, result, decided))), ...);
        return result;
    }

    template <typename P, typename T>
    static constexpr bool invoke_compare(const P &p, const T &a, const T &b, bool &result, bool &decided) {
...
template <typename... Projs> [[nodiscard]] constexpr auto multi_sort(Projs... projs) {
...
struct partial_sort_view : pipeable_base<partial_sort_view<Proj, Comp>> {
    ...
    template <input_range R> constexpr auto operator()(R &&r) const {
        ...
        if (descending) {
            std::partial_sort(result.begin(), result.begin() + count, result.end(), [this](const T &a, const T &b) {
