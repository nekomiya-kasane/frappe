...
struct comparator {
    ...
    [[nodiscard]] constexpr auto desc() const {
        return comparator<Proj, Comp>{proj, comp, true};
    }
    
    [[nodiscard]] constexpr auto asc() const {
        return comparator<Proj, Comp>{proj, comp, false};
    }
};

template<typename Proj, typename Comp>
comparator(Proj, Comp, bool) -> comparator<Proj, Comp>;

template<typename Proj>
comparator(Proj) -> comparator<Proj, std::less<>>;
...
struct multi_comparator {
    std::tuple<Comps...> comps;
    
    constexpr explicit multi_comparator(Comps... cs) 
        : comps(std::move(cs)...) {}
    
    constexpr explicit multi_comparator(std::tuple<Comps...> t)
        : comps(std::move(t)) {}
...
private:
    ...
    constexpr bool compare_impl(const T& a, const T& b, std::index_sequence<Is...>) const {
        ...
        bool decided = false;
        ((decided || (
            std::get<Is>(comps)(a, b) ? (result = true, decided = true) :
            std::get<Is>(comps)(b, a) ? (decided = true) : false
        )), ...);
        return result;
    }
};
...
template<typename... Comps>
multi_comparator(Comps...) -> multi_comparator<Comps...>;

// CPO: rpp::multi_compare
template<typename... Comps>
[[nodiscard]] constexpr auto multi_compare(Comps... comps) {
    ...
    static bool natural_compare(std::string_view a, std::string_view b) {
        ...
        while (ai != ae && bi != be) {
            if (std::isdigit(static_cast<unsigned char>(*ai)) && 
                ...
                while (a_len > 1 && *a_start == '0') { ++a_start; --a_len; }
                while (b_len > 1 && *b_start == '0') { ++b_start; --b_len; }
                
                if (a_len != b_len) return a_len < b_len;
                
                auto cmp = std::string_view(&*a_start, a_len).compare(
                    std::string_view(&*b_start, b_len));
...
template<typename Proj>
natural_comparator_by(Proj) -> natural_comparator_by<Proj>;
...
template<typename Comp>
[[nodiscard]] constexpr auto reverse(Comp comp) {
