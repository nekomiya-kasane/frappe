...
template <typename Proj = std::identity> struct distinct_view : pipeable_base<distinct_view<Proj>> {
    Proj proj;
...
template <typename Proj> distinct_view(Proj) -> distinct_view<Proj>;

inline constexpr struct distinct_fn {
    [[nodiscard]] constexpr auto operator()() const { return distinct_view<>{}; }
...
inline constexpr struct unique_consecutive_fn {
    [[nodiscard]] constexpr auto operator()() const {
        return make_closure([](auto &&r) { return std::forward<decltype(r)>(r) | ::ranges::views::unique; });
    }

    template <typename Proj> [[nodiscard]] constexpr auto operator()(Proj proj) const {
        return make_closure([p = std::move(proj)](auto &&r) {
            return std::forward<decltype(r)>(r) | ::ranges::views::unique([p](const auto &a, const auto &b) {
...
template <typename Proj = std::identity> struct distinct_count_view : pipeable_base<distinct_count_view<Proj>> {
    Proj proj;
...
inline constexpr struct distinct_count_fn {
    [[nodiscard]] constexpr auto operator()() const { return distinct_count_view<>{}; }
