...
struct avg_view : pipeable_base<avg_view<Proj>> {
    Proj proj;
...
template<typename Proj = std::identity>
struct max_by_view : pipeable_base<max_by_view<Proj>> {
    Proj proj;
...
template<typename Proj = std::identity>
struct min_by_view : pipeable_base<min_by_view<Proj>> {
    Proj proj;
...
inline constexpr struct exists_fn {
    ...
    [[nodiscard]] constexpr auto operator()(Pred pred) const {
        return make_closure([p=std::move(pred)](auto&& r) {
            return ::ranges::any_of(std::forward<decltype(r)>(r), p);
        });
    }
} exists{};
...
inline constexpr struct all_fn {
    ...
    [[nodiscard]] constexpr auto operator()(Pred pred) const {
        return make_closure([p=std::move(pred)](auto&& r) {
            return ::ranges::all_of(std::forward<decltype(r)>(r), p);
        });
    }
} all{};
...
inline constexpr struct none_fn {
    ...
    [[nodiscard]] constexpr auto operator()(Pred pred) const {
        return make_closure([p=std::move(pred)](auto&& r) {
            return ::ranges::none_of(std::forward<decltype(r)>(r), p);
        });
    }
} none{};
...
template<typename Proj = std::identity>
struct variance_view : pipeable_base<variance_view<Proj>> {
    Proj proj;
    ...
    template<input_range R>
    constexpr double operator()(R&& r) const {
        double sum = 0;
        double sum_sq = 0;
        std::size_t count = 0;
...
inline constexpr struct variance_fn {
    ...
    template<typename Proj>
    [[nodiscard]] constexpr auto operator()(Proj proj, bool sample = false) const {
        return variance_view<Proj>{std::move(proj), sample};
    }
} variance{};

template<typename Proj = std::identity>
struct stddev_view : pipeable_base<stddev_view<Proj>> {
    Proj proj;
    bool sample = false;
...
template<typename Proj = std::identity>
struct median_view : pipeable_base<median_view<Proj>> {
    Proj proj;
...
template<typename Proj = std::identity>
struct percentile_view : pipeable_base<percentile_view<Proj>> {
    Proj proj;
    double p;  // 0.0 to 1.0
    
    constexpr percentile_view(double percentile, Proj pr = {}) 
        : proj(std::move(pr)), p(percentile) {}
...
inline constexpr struct percentile_fn {
    [[nodiscard]] constexpr auto operator()(double p) const { 
        return percentile_view<>{p}; 
    }
    
    template<typename Proj>
    [[nodiscard]] constexpr auto operator()(double p, Proj proj) const {
