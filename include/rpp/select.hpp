...
template <typename... Projs> struct select_view : pipeable_base<select_view<Projs...>> {
    ...
    template <input_range R> constexpr auto operator()(R &&r) const {
        return std::forward<R>(r) | ::ranges::views::transform([this](const auto &x) {
                   return std::apply([&x](const auto &...ps) { return std::make_tuple(std::invoke(ps, x)...); }, projs);
               });
    }
};

template <typename... Projs> select_view(Projs...) -> select_view<Projs...>;

inline constexpr struct select_fn {
    template <typename... Projs> [[nodiscard]] constexpr auto operator()(Projs... projs) const {
...
template <typename Proj> struct pluck_view : pipeable_base<pluck_view<Proj>> {
    Proj proj;

    constexpr explicit pluck_view(Proj p) : proj(std::move(p)) {}

    template <input_range R> constexpr auto operator()(R &&r) const {
        return std::forward<R>(r) | ::ranges::views::transform([p = proj](const auto &x) { return std::invoke(p, x); });
    }
};

template <typename Proj> pluck_view(Proj) -> pluck_view<Proj>;
...
template <typename Proj> struct with_view : pipeable_base<with_view<Proj>> {
    Proj proj;

    constexpr explicit with_view(Proj p) : proj(std::move(p)) {}

    template <input_range R> constexpr auto operator()(R &&r) const {
        return std::forward<R>(r) | ::ranges::views::transform([p = proj](auto &&x) {
...
template <typename Cond, typename Then, typename Else> struct case_when_proj {
    Cond cond;
    Then then_proj;
    Else else_proj;
...
inline constexpr struct case_when_fn {
    ...
    [[nodiscard]] constexpr auto operator()(Cond cond, Then then_proj, Else else_proj) const {
        return case_when_proj<Cond, Then, Else>{std::move(cond), std::move(then_proj), std::move(else_proj)};
    }
} case_when{};
...
template <typename Proj, typename Default> struct coalesce_proj {
    Proj proj;
