...
template <typename Fn> struct map_view : pipeable_base<map_view<Fn>> {
    Fn fn;
...
template <typename Fn> map_view(Fn) -> map_view<Fn>;

inline constexpr struct map_fn {
    template <typename Fn> [[nodiscard]] constexpr auto operator()(Fn fn) const { return map_view<Fn>{std::move(fn)}; }
} map{};
...
template <typename Fn> struct flat_map_view : pipeable_base<flat_map_view<Fn>> {
    Fn fn;

    constexpr explicit flat_map_view(Fn f) : fn(std::move(f)) {}

    template <input_range R> constexpr auto operator()(R &&r) const {
        return std::forward<R>(r) | ::ranges::views::transform(fn) | ::ranges::views::join;
    }
};
...
inline constexpr struct flatten_fn {
    [[nodiscard]] constexpr auto operator()() const {
        return make_closure([](auto &&r) { return std::forward<decltype(r)>(r) | ::ranges::views::join; });
    }
} flatten{};
...
inline constexpr struct reverse_range_fn {
    [[nodiscard]] constexpr auto operator()() const {
        return make_closure([](auto &&r) { return std::forward<decltype(r)>(r) | ::ranges::views::reverse; });
    }
} reverse_range{};
...
template <typename Fn> struct tap_view : pipeable_base<tap_view<Fn>> {
    Fn fn;

    constexpr explicit tap_view(Fn f) : fn(std::move(f)) {}

    template <input_range R> constexpr auto operator()(R &&r) const {
        return std::forward<R>(r) | ::ranges::views::transform([f = fn](auto &&x) -> decltype(auto) {
                   f(x);
...
inline constexpr struct tap_fn {
    template <typename Fn> [[nodiscard]] constexpr auto operator()(Fn fn) const { return tap_view<Fn>{std::move(fn)}; }
} tap{};
...
template <typename Fn> struct filter_map_view : pipeable_base<filter_map_view<Fn>> {
    ...
    template <input_range R> constexpr auto operator()(R &&r) const {
        ...
        return std::forward<R>(r) | ::ranges::views::transform(fn) |
               ...
               ::ranges::views::transform(
                   [](auto &&opt) -> decltype(auto) { return *std::forward<decltype(opt)>(opt); });
    }
};
...
template <typename T> struct intersperse_view : pipeable_base<intersperse_view<T>> {
    T separator;
...
inline constexpr struct intersperse_fn {
    template <typename T> [[nodiscard]] constexpr auto operator()(T separator) const {
...
template <typename BinaryOp, typename Init> struct scan_view : pipeable_base<scan_view<BinaryOp, Init>> {
    BinaryOp op;
    Init init;
    ...
    template <input_range R> constexpr auto operator()(R &&r) const {
        ...
        return std::forward<R>(r) | ::ranges::views::transform([acc, operation](auto &&item) mutable {
...
inline constexpr struct scan_fn {
    template <typename BinaryOp, typename Init> [[nodiscard]] constexpr auto operator()(Init init, BinaryOp op) const {
...
template <typename T> struct repeat_view : pipeable_base<repeat_view<T>> {
    std::size_t n;
...
inline constexpr struct repeat_fn {
    [[nodiscard]] constexpr auto operator()(std::size_t n) const { return repeat_view<void>{n}; }
} repeat{};
...
template <typename RNG = std::mt19937> struct shuffle_view : pipeable_base<shuffle_view<RNG>> {
    mutable RNG rng;
...
inline constexpr struct shuffle_fn {
    [[nodiscard]] auto operator()() const { return shuffle_view<>{}; }

    [[nodiscard]] auto operator()(unsigned int seed) const { return shuffle_view<std::mt19937>{seed}; }

    template <typename RNG> [[nodiscard]] auto operator()(RNG rng) const { return shuffle_view<RNG>{std::move(rng)}; }
} shuffle{};
...
template <typename RNG = std::mt19937> struct sample_view : pipeable_base<sample_view<RNG>> {
    std::size_t n;
    mutable RNG rng;
...
inline constexpr struct sample_fn {
    [[nodiscard]] auto operator()(std::size_t n) const { return sample_view<>{n}; }

    [[nodiscard]] auto operator()(std::size_t n, unsigned int seed) const { return sample_view<std::mt19937>{n, seed}; }
} sample{};
