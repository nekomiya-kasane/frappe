...
concept projection_for = std::invocable<Proj, T> && 
                         !std::same_as<std::invoke_result_t<Proj, T>, void>;
...
template<typename R>
concept bidirectional_range = std::ranges::bidirectional_range<R> || 
                               ::ranges::bidirectional_range<R>;

template<typename R>
concept random_access_range = std::ranges::random_access_range<R> || 
...
template<typename A, typename R>
concept terminal_adapter_for = input_range<R> && requires(A a, R&& r) {
    a(std::forward<R>(r));
};
