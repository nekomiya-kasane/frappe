...
template <typename R2, typename Proj = std::identity> struct union_view : pipeable_base<union_view<R2, Proj>> {
    R2 other;
    Proj proj;
...
inline constexpr struct union_fn {
    ...
    template <input_range R2, typename Proj> [[nodiscard]] constexpr auto operator()(R2 &&other, Proj proj) const {
        using R2Decayed = std::decay_t<R2>;
        return union_view<R2Decayed, Proj>{std::forward<R2>(other) | ::ranges::to<std::vector>(), std::move(proj)};
    }
} union_{};
...
template <typename R2, typename Proj = std::identity> struct intersect_view : pipeable_base<intersect_view<R2, Proj>> {
    R2 other;
    Proj proj;
...
inline constexpr struct intersect_fn {
    ...
    template <input_range R2, typename Proj> [[nodiscard]] constexpr auto operator()(R2 &&other, Proj proj) const {
        using R2Decayed = std::decay_t<R2>;
        return intersect_view<R2Decayed, Proj>{std::forward<R2>(other) | ::ranges::to<std::vector>(), std::move(proj)};
    }
} intersect{};
...
template <typename R2, typename Proj = std::identity> struct except_view : pipeable_base<except_view<R2, Proj>> {
    R2 other;
    Proj proj;
...
inline constexpr struct except_fn {
    ...
    template <input_range R2, typename Proj> [[nodiscard]] constexpr auto operator()(R2 &&other, Proj proj) const {
        using R2Decayed = std::decay_t<R2>;
        return except_view<R2Decayed, Proj>{std::forward<R2>(other) | ::ranges::to<std::vector>(), std::move(proj)};
    }
} except{};
...
inline constexpr struct symmetric_difference_fn {
    template <input_range R2> [[nodiscard]] constexpr auto operator()(R2 &&other) const {
        using R2Decayed = std::decay_t<R2>;
        return symmetric_difference_view<R2Decayed>{std::forward<R2>(other) | ::ranges::to<std::vector>()};
    }

    template <input_range R2, typename Proj> [[nodiscard]] constexpr auto operator()(R2 &&other, Proj proj) const {
        using R2Decayed = std::decay_t<R2>;
        return symmetric_difference_view<R2Decayed, Proj>{std::forward<R2>(other) | ::ranges::to<std::vector>(),
                                                          std::move(proj)};
    }
