...
struct sum_view : pipeable_base<sum_view<Proj>> {
    Proj proj;
...
template<typename Proj = std::identity>
struct product_view : pipeable_base<product_view<Proj>> {
    Proj proj;
...
inline constexpr struct enumerate_fn {
    [[nodiscard]] constexpr auto operator()() const {
        return make_closure([](auto&& r) {
            return std::forward<decltype(r)>(r) | ::ranges::views::enumerate;
        });
    }
...
template<typename Proj>
struct group_by_view : pipeable_base<group_by_view<Proj>> {
    Proj proj;
...
template<typename Proj>
struct count_by_view : pipeable_base<count_by_view<Proj>> {
    Proj proj;
...
struct to_map_view : pipeable_base<to_map_view<KeyProj, ValueProj>> {
    ...
    constexpr to_map_view(KeyProj kp, ValueProj vp = {}) 
...
struct fold_view : pipeable_base<fold_view<BinaryOp, Init>> {
    BinaryOp op;
    Init init;
...
inline constexpr struct reduce_fn {
    ...
    [[nodiscard]] constexpr auto operator()(BinaryOp op) const {
