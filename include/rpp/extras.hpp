...
[[nodiscard]] constexpr auto from(Container&& c) {
...
struct for_each_view : pipeable_base<for_each_view<Fn>> {
    Fn fn;
...
struct join_str_view : pipeable_base<join_str_view> {
    ...
    template<input_range R>
    auto operator()(R&& r) const -> std::string {
...
inline constexpr struct join_str_fn {
    [[nodiscard]] auto operator()(std::string sep = ", ") const {
        return join_str_view{std::move(sep)};
    }
} join_str{};
...
struct format_table_view : pipeable_base<format_table_view<Headers...>> {
    ...
    auto operator()(R&& r) const -> std::string {
        ...
        for (auto&& item : r) {
            ...
            if constexpr (requires { std::tuple_size<T>::value; }) {
                std::apply([&](const auto&... fields) {
                    ((row.push_back((std::ostringstream{} << fields).str())), ...);
                }, item);
            } else {
                std::ostringstream oss;
                oss << item;
        ...
        if constexpr (sizeof...(Headers) > 0) {
            ...
            for (std::size_t i = 0; i < rows[0].size(); ++i) {
                out << ' ' << std::left << std::setw(static_cast<int>(col_widths[i])) 
                    << rows[0][i] << " |";
            }
            out << '\n';
            start = 1;
...
inline constexpr struct format_table_fn {
    [[nodiscard]] auto operator()() const {
        return format_table_view<>{};
    }
    
    template<typename... Headers>
    [[nodiscard]] auto operator()(Headers... headers) const {
...
struct group_aggregate_view : pipeable_base<group_aggregate_view<KeyProj, Aggs...>> {
    ...
    constexpr group_aggregate_view(KeyProj kp, Aggs... as)
        : key_proj(std::move(kp)), aggs(std::move(as)...) {}
    ...
    constexpr auto operator()(R&& r) const {
        ...
        for (auto&& item : r) {
            groups[std::invoke(key_proj, item)].push_back(
                std::forward<decltype(item)>(item));
        }
        
        // Apply aggregates to each group
        using ResultTuple = std::tuple<Key, 
            decltype(std::declval<Aggs>()(std::declval<std::vector<T>&>()))...>;
        
        std::vector<ResultTuple> result;
        result.reserve(groups.size());
        
        for (auto& [key, items] : groups) {
            result.push_back(std::apply([&](const auto&... agg) {
                return std::make_tuple(key, agg(items)...);
            }, aggs));
        }
        return result;
    }
...
inline constexpr struct group_aggregate_fn {
    ...
    [[nodiscard]] constexpr auto operator()(KeyProj key_proj, Aggs... aggs) const {
        return group_aggregate_view<KeyProj, Aggs...>{
            std::move(key_proj), std::move(aggs)...};
    }
} group_aggregate{};
...
struct having_view : pipeable_base<having_view<Pred>> {
    Pred pred;
...
struct sliding_window_view : pipeable_base<sliding_window_view<T>> {
    ...
    constexpr auto operator()(R&& r) const {
        ...
        if (items.size() >= window_size) {
            ...
            for (std::size_t i = 0; i + window_size <= items.size(); ++i) {
                result.emplace_back(items.begin() + i, 
                                    items.begin() + i + window_size);
            }
        }
        return result;
    }
...
template<typename R2, typename Fn>
struct zip_with_view : pipeable_base<zip_with_view<R2, Fn>> {
    R2 other;
    Fn fn;
...
inline constexpr struct zip_with_fn {
    template<input_range R2, typename Fn>
    [[nodiscard]] constexpr auto operator()(R2&& other, Fn fn) const {
        return zip_with_view<std::decay_t<R2>, Fn>{
...
template<typename Fn>
struct enumerate_map_view : pipeable_base<enumerate_map_view<Fn>> {
    Fn fn;
...
template<typename Fn>
struct batch_view : pipeable_base<batch_view<Fn>> {
...
inline constexpr struct batch_fn {
    template<typename Fn>
    [[nodiscard]] constexpr auto operator()(std::size_t n, Fn fn) const {
...
template<typename Pred>
struct take_while_view : pipeable_base<take_while_view<Pred>> {
    Pred pred;
...
template<typename Pred>
struct drop_while_view : pipeable_base<drop_while_view<Pred>> {
    Pred pred;
...
template<typename Fn>
[[nodiscard]] auto generate(Fn fn, std::size_t n) {
...
template<typename Fn>
[[nodiscard]] auto generate_n(Fn fn, std::size_t n) {
...
[[nodiscard]] auto iota(T start, T end_val) {
    std::vector<T> result;
    if (start < end_val) {
