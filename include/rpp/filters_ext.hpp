...
[[nodiscard]] inline auto like(Proj proj, std::string_view pattern) {
...
template<typename Proj>
[[nodiscard]] inline auto not_like(Proj proj, std::string_view pattern) {
    auto regex_pattern = detail::like_to_regex(pattern);
    return filter([p=std::move(proj), re=std::regex(regex_pattern)](const auto& x) {
        auto s = std::invoke(p, x);
        return !std::regex_match(s.begin(), s.end(), re);
...
template<typename Proj>
[[nodiscard]] inline auto matches(Proj proj, std::string_view pattern) {
...
template<typename Proj>
[[nodiscard]] inline auto matches(Proj proj, const std::regex& re) {
...
template<typename Proj>
[[nodiscard]] inline auto full_match(Proj proj, std::string_view pattern) {
    return filter([p=std::move(proj), re=std::regex(std::string(pattern))](const auto& x) {
        auto s = std::invoke(p, x);
        return std::regex_match(s.begin(), s.end(), re);
    });
...
[[nodiscard]] constexpr auto is_type(Proj proj) {
...
[[nodiscard]] constexpr auto all_of(Preds... preds) {
    return filter([...ps=std::move(preds)](const auto& x) {
        return (ps(x) && ...);
    });
}
...
template<typename... Preds>
[[nodiscard]] constexpr auto none_of(Preds... preds) {
    return filter([...ps=std::move(preds)](const auto& x) {
        return !(ps(x) || ...);
    });
}
