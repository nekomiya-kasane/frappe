...
struct traversal_options {
    ...
    // Builder pattern
    traversal_options &with_follow_symlinks(bool v = true) {
        follow_symlinks = v;
        return *this;
    }
    traversal_options &with_skip_permission_denied(bool v = true) {
        skip_permission_denied = v;
        return *this;
    }
    traversal_options &with_include_hidden(bool v = true) {
        include_hidden = v;
        return *this;
    }
    traversal_options &with_max_depth(int d) {
        max_depth = d;
        return *this;
    }
    traversal_options &with_cross_device(bool v = true) {
        cross_device = v;
        return *this;
    }
    traversal_options &with_max_entries(std::size_t n) {
        max_entries = n;
        return *this;
    }
    traversal_options &with_cycle_detection(cycle_detection s) {
        cycle_strategy = s;
        return *this;
    }
    traversal_options &with_on_cycle(on_cycle a) {
        cycle_action = a;
        return *this;
    }
...
[[nodiscard]] std::generator<file_entry> traverse(const path &root, const traversal_options &opts = {});
#endif

// Traverse and collect all entries
[[nodiscard]] FRAPPE_API std::vector<file_entry> traverse_collect(const path &root, const traversal_options &opts = {});

// Traverse with callback
FRAPPE_API void traverse_with(const path &root, const std::function<bool(const file_entry &)> &callback,
                              const traversal_options &opts = {});

// Parallel traversal (returns collected entries)
[[nodiscard]] FRAPPE_API std::vector<file_entry> traverse_parallel(const path &root, const traversal_options &opts = {},
...
[[nodiscard]] inline safe_recursive_directory_range safe_recursive_directory(const path &root,
                                                                             const traversal_options &opts = {}) {
