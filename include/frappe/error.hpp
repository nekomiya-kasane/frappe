...
namespace frappe {
    ...
    template <typename T> T &throw_if_error(result<T> &res, const std::filesystem::path &p = {}) {
        if (!res) {
            if (p.empty()) {
    ...
    template <typename T> const T &throw_if_error(const result<T> &res, const std::filesystem::path &p = {}) {
        if (!res) {
            if (p.empty()) {
                throw std::filesystem::filesystem_error("operation failed", res.error());
            }
    ...
    class path_error : public filesystem_error {
      public:
        path_error(const std::string &what_arg, error ec) : filesystem_error(what_arg, ec) {}
    ...
    class glob_error : public std::runtime_error {
      public:
        explicit glob_error(const std::string &what_arg) : std::runtime_error(what_arg) {}
    };
