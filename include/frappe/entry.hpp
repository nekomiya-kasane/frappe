...
struct file_entry {
    ...
    auto operator<=>(const file_entry& other) const noexcept {
        return file_path <=> other.file_path;
    }
    bool operator==(const file_entry& other) const noexcept {
        return file_path == other.file_path;
    }
};
...
[[nodiscard]] FRAPPE_API std::generator<file_entry> enumerate_entries_recursive(const path& dir, const list_options& opts);
#endif
