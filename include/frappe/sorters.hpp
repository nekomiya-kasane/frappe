...
struct sort_adapter {
    ...
    template<std::ranges::input_range R>
    friend auto operator|(R&& r, const sort_adapter& s) {
        return s(std::forward<R>(r));
    }
    
    // Create descending version
    [[nodiscard]] auto desc() const {
        return sort_adapter<Proj, Comp>{projection, comparator, true};
    }
    
    // Create ascending version
    [[nodiscard]] auto asc() const {
        return sort_adapter<Proj, Comp>{projection, comparator, false};
    }
};

template<typename Proj, typename Comp = std::less<>>
[[nodiscard]] inline auto sort_by(Proj proj, Comp comp = {}) {
...
struct multi_sort_adapter {
    ...
    template<std::ranges::input_range R>
    friend auto operator|(R&& r, const multi_sort_adapter& s) {
        return s(std::forward<R>(r));
    }
...
private:
    ...
    bool compare_multi(const T& a, const T& b, std::index_sequence<Is...>) const {
        ...
        bool decided = false;
        ((decided || (
            std::get<Is>(projections)(a) != std::get<Is>(projections)(b) 
...
template<typename... Projs>
[[nodiscard]] inline auto sort_by_multi(Projs... projs) {
...
struct stable_sort_adapter {
    ...
    auto operator()(R&& r) const {
        ...
        if (descending) {
            std::ranges::stable_sort(result, [this](const auto& a, const auto& b) {
                return comparator(projection(b), projection(a));
            });
        } else {
            std::ranges::stable_sort(result, [this](const auto& a, const auto& b) {
    ...
    template<std::ranges::input_range R>
    friend auto operator|(R&& r, const stable_sort_adapter& s) {
        return s(std::forward<R>(r));
    }
};
...
namespace entry {
    struct reverse_adapter {
        ...
        template<std::ranges::input_range R>
        friend auto operator|(R&& r, const reverse_adapter& rev) {
...
struct natural_sort_adapter {
    ...
    auto operator()(R&& r) const {
        ...
        if (descending) {
            std::ranges::sort(result, [](const auto& a, const auto& b) {
                return detail::natural_less(b.name, a.name);
            });
        } else {
            std::ranges::sort(result, [](const auto& a, const auto& b) {
                return detail::natural_less(a.name, b.name);
            });
        }
        return result;
    }
    ...
    template<std::ranges::input_range R>
    friend auto operator|(R&& r, const natural_sort_adapter& s) {
        return s(std::forward<R>(r));
    }
    
    [[nodiscard]] auto desc() const {
        return natural_sort_adapter{true};
    }
};
...
namespace entry {
    // Depth sorting
    inline const auto sort_by_depth = sort_by([](const file_entry& e) {
        return std::distance(e.file_path.begin(), e.file_path.end());
    });
    inline const auto sort_by_depth_desc = sort_by_desc([](const file_entry& e) {
        return std::distance(e.file_path.begin(), e.file_path.end());
    });
    
    // Parent path sorting
    inline const auto sort_by_parent = sort_by([](const file_entry& e) {
