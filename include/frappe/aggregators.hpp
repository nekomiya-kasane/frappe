...
namespace frappe {
    ...
    namespace entry {
        struct first_adapter {
            ...
            template <std::ranges::input_range R> friend auto operator|(R &&r, const first_adapter &f) {
    ...
    struct last_adapter {
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const last_adapter &l) {
    ...
    namespace entry {
        struct take_adapter {
            ...
            template <std::ranges::input_range R> friend auto operator|(R &&r, const take_adapter &t) {
    ...
    struct skip_adapter {
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const skip_adapter &s) {
    ...
    template <typename Pred> struct take_while_adapter {
        Pred predicate;
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const take_while_adapter &t) {
            return t(std::forward<R>(r));
        }
    };

    template <typename Pred> [[nodiscard]] inline take_while_adapter<Pred> take_while(Pred pred) {
        return take_while_adapter<Pred>{std::move(pred)};
    }

    // Drop while predicate is true
    template <typename Pred> struct drop_while_adapter {
        Pred predicate;
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const drop_while_adapter &d) {
            return d(std::forward<R>(r));
        }
    };

    template <typename Pred> [[nodiscard]] inline drop_while_adapter<Pred> drop_while(Pred pred) {
    ...
    struct count_adapter {
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const count_adapter &c) {
    ...
    template <typename Pred> struct count_if_adapter {
        Pred predicate;
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const count_if_adapter &c) {
            return c(std::forward<R>(r));
        }
    };

    template <typename Pred> [[nodiscard]] inline count_if_adapter<Pred> count_if(Pred pred) {
    ...
    namespace entry {
        struct total_size_adapter {
            template <std::ranges::input_range R> auto operator()(R &&r) const -> std::uintmax_t {
            ...
            template <std::ranges::input_range R> friend auto operator|(R &&r, const total_size_adapter &t) {
                return t(std::forward<R>(r));
            }
        };
    ...
    struct avg_size_adapter {
        template <std::ranges::input_range R> auto operator()(R &&r) const -> double {
            std::uintmax_t total = 0;
            std::size_t count = 0;
            for (const auto &item : r) {
                if constexpr (std::is_same_v<std::decay_t<decltype(item)>, file_entry>) {
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const avg_size_adapter &a) {
    ...
    // Group by projection
    template <typename Proj> struct group_by_adapter {
        Proj projection;
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const group_by_adapter &g) {
            return g(std::forward<R>(r));
        }
    };

    template <typename Proj> [[nodiscard]] inline group_by_adapter<Proj> group_by(Proj proj) {
        return group_by_adapter<Proj>{std::move(proj)};
    }

    // Count by projection
    template <typename Proj> struct count_by_adapter {
        Proj projection;
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const count_by_adapter &c) {
            return c(std::forward<R>(r));
        }
    };

    template <typename Proj> [[nodiscard]] inline count_by_adapter<Proj> count_by(Proj proj) {
        return count_by_adapter<Proj>{std::move(proj)};
    }

    // Sum by projection (e.g., total size per extension)
    template <typename KeyProj, typename ValueProj> struct sum_by_adapter {
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const sum_by_adapter &s) {
            return s(std::forward<R>(r));
        }
    };
    ...
    template <typename Proj> struct min_by_adapter {
        Proj projection;
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const min_by_adapter &m) {
            return m(std::forward<R>(r));
        }
    };

    template <typename Proj> [[nodiscard]] inline min_by_adapter<Proj> min_by(Proj proj) {
        return min_by_adapter<Proj>{std::move(proj)};
    }

    template <typename Proj> struct max_by_adapter {
        Proj projection;
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const max_by_adapter &m) {
            return m(std::forward<R>(r));
        }
    };

    template <typename Proj> [[nodiscard]] inline max_by_adapter<Proj> max_by(Proj proj) {
    ...
    struct collect_adapter {
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const collect_adapter &c) {
            return c(std::forward<R>(r));
        }
    };
    ...
    template <typename Pred> struct any_adapter {
        Pred predicate;
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const any_adapter &a) {
            return a(std::forward<R>(r));
        }
    };

    template <typename Pred> [[nodiscard]] inline any_adapter<Pred> any_of(Pred pred) {
        return any_adapter<Pred>{std::move(pred)};
    }

    template <typename Pred> struct all_adapter {
        Pred predicate;
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const all_adapter &a) {
            return a(std::forward<R>(r));
        }
    };

    template <typename Pred> [[nodiscard]] inline all_adapter<Pred> all_of(Pred pred) {
        return all_adapter<Pred>{std::move(pred)};
    }

    template <typename Pred> struct none_adapter {
        Pred predicate;
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const none_adapter &n) {
            return n(std::forward<R>(r));
        }
    };

    template <typename Pred> [[nodiscard]] inline none_adapter<Pred> none_of(Pred pred) {
    ...
    template <typename Pred> struct partition_adapter {
        Pred predicate;
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const partition_adapter &p) {
            return p(std::forward<R>(r));
        }
    };

    template <typename Pred> [[nodiscard]] inline partition_adapter<Pred> partition_by(Pred pred) {
    ...
    template <typename Proj> struct unique_by_adapter {
        Proj projection;
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const unique_by_adapter &u) {
            return u(std::forward<R>(r));
        }
    };

    template <typename Proj> [[nodiscard]] inline unique_by_adapter<Proj> unique_by(Proj proj) {
    ...
    struct unique_adapter {
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const unique_adapter &u) {
            return u(std::forward<R>(r));
        }
    };
    ...
    struct chunk_adapter {
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const chunk_adapter &c) {
            return c(std::forward<R>(r));
        }
    };
    ...
    struct enumerate_adapter {
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const enumerate_adapter &e) {
    ...
    struct nth_adapter {
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const nth_adapter &n) {
            return n(std::forward<R>(r));
        }
    };
    ...
    struct sample_adapter {
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const sample_adapter &s) {
            return s(std::forward<R>(r));
        }
    };
    ...
    struct flatten_adapter {
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const flatten_adapter &f) {
            return f(std::forward<R>(r));
        }
    };
    ...
    template <typename T> struct intersperse_adapter {
        T separator;
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const intersperse_adapter &i) {
            return i(std::forward<R>(r));
        }
    };

    template <typename T> [[nodiscard]] inline intersperse_adapter<T> intersperse(T sep) {
    ...
    template <typename Proj> struct dedup_by_adapter {
        Proj projection;
        ...
        template <std::ranges::input_range R> friend auto operator|(R &&r, const dedup_by_adapter &d) {
            return d(std::forward<R>(r));
        }
    };

    template <typename Proj> [[nodiscard]] inline dedup_by_adapter<Proj> dedup_by(Proj proj) {
