...
inline auto list_disks_sender() {
    return stdexec::just() | stdexec::then([]() { return list_disks(); });
}

// Get disk info asynchronously
inline auto get_disk_info_sender(std::string device_path) {
    return stdexec::just(std::move(device_path)) | stdexec::then([](std::string path) { return get_disk_info(path); });
}

// List partitions asynchronously
inline auto list_partitions_sender() {
    return stdexec::just() | stdexec::then([]() { return list_partitions(); });
}
...
inline auto get_partition_table_sender(std::string device_path) {
    return stdexec::just(std::move(device_path)) |
           stdexec::then([](std::string path) { return get_partition_table(path); });
}

// List mounts asynchronously
inline auto list_mounts_sender() {
    return stdexec::just() | stdexec::then([]() { return list_mounts(); });
}

// Get file owner SID asynchronously (Windows)
inline auto get_file_owner_sid_sender(path p) {
    return stdexec::just(std::move(p)) | stdexec::then([](path file_path) { return get_file_owner_sid(file_path); });
}

// Get permissions asynchronously
inline auto get_permissions_sender(path p) {
    return stdexec::just(std::move(p)) | stdexec::then([](path file_path) { return get_permissions(file_path); });
}
...
inline auto check_access_sender(path p, access_rights rights) {
    return stdexec::just(std::move(p), rights) |
           stdexec::then([](path file_path, access_rights r) { return check_access(file_path, r); });
}
...
inline auto get_permissions_bulk_sender(std::vector<path> paths) {
    return stdexec::just(std::move(paths)) | stdexec::then([](std::vector<path> file_paths) {
...
inline auto check_access_bulk_sender(std::vector<path> paths, access_rights rights) {
    return stdexec::just(std::move(paths), rights) | stdexec::then([](std::vector<path> file_paths, access_rights r) {
...
template <typename Sender> auto on_thread_pool(Sender &&sender) {
    return stdexec::on(get_default_scheduler(), std::forward<Sender>(sender));
}

// Execute sender and wait for result
template <typename Sender> auto sync_wait(Sender &&sender) {
...
inline auto list_network_shares_sender() {
    return stdexec::just() | stdexec::then([]() { return list_network_shares(); });
}

// Get network share info asynchronously
inline auto get_network_share_info_sender(path p) {
    return stdexec::just(std::move(p)) |
           stdexec::then([](path file_path) { return get_network_share_info(file_path); });
}

// Check if network path asynchronously
inline auto is_network_path_sender(path p) {
    return stdexec::just(std::move(p)) | stdexec::then([](path file_path) { return is_network_path(file_path); });
}

#ifdef _WIN32
// List mapped drives asynchronously (Windows only)
inline auto list_mapped_drives_sender() {
    return stdexec::just() | stdexec::then([]() { return list_mapped_drives(); });
}
#endif
...
inline auto list_removable_devices_sender() {
    return stdexec::just() | stdexec::then([]() { return list_removable_devices(); });
}

// Get removable device info asynchronously
inline auto get_removable_device_info_sender(path p) {
    return stdexec::just(std::move(p)) |
           stdexec::then([](path device_path) { return get_removable_device_info(device_path); });
}

// Check if device is ready asynchronously
inline auto is_device_ready_sender(path p) {
    return stdexec::just(std::move(p)) | stdexec::then([](path device_path) { return is_device_ready(device_path); });
}

// Check if media is present asynchronously
inline auto is_media_present_sender(path p) {
    return stdexec::just(std::move(p)) | stdexec::then([](path device_path) { return is_media_present(device_path); });
}
...
inline auto is_cloud_path_sender(path p) {
    return stdexec::just(std::move(p)) | stdexec::then([](path file_path) { return is_cloud_path(file_path); });
}

// Get cloud provider asynchronously
inline auto get_cloud_provider_sender(path p) {
    return stdexec::just(std::move(p)) | stdexec::then([](path file_path) { return get_cloud_provider(file_path); });
}

// Get cloud storage info asynchronously
inline auto get_cloud_storage_info_sender(path p) {
    return stdexec::just(std::move(p)) |
           stdexec::then([](path file_path) { return get_cloud_storage_info(file_path); });
}

// List cloud mounts asynchronously
inline auto list_cloud_mounts_sender() {
    return stdexec::just() | stdexec::then([]() { return list_cloud_mounts(); });
}
...
inline auto is_virtual_filesystem_sender(path p) {
    return stdexec::just(std::move(p)) | stdexec::then([](path file_path) { return is_virtual_filesystem(file_path); });
}

// Check if FUSE mount asynchronously
inline auto is_fuse_mount_sender(path p) {
    return stdexec::just(std::move(p)) | stdexec::then([](path file_path) { return is_fuse_mount(file_path); });
}

// Check if encrypted mount asynchronously
inline auto is_encrypted_mount_sender(path p) {
    return stdexec::just(std::move(p)) | stdexec::then([](path file_path) { return is_encrypted_mount(file_path); });
}

// Get virtual filesystem info asynchronously
inline auto get_virtual_fs_info_sender(path p) {
    return stdexec::just(std::move(p)) | stdexec::then([](path file_path) { return get_virtual_fs_info(file_path); });
}
...
inline auto get_filesystem_type_sender(path p) {
    return stdexec::just(std::move(p)) | stdexec::then([](path file_path) { return get_filesystem_type(file_path); });
}

// Get filesystem features asynchronously
inline auto get_filesystem_features_sender(path p) {
    return stdexec::just(std::move(p)) |
           stdexec::then([](path file_path) { return get_filesystem_features(file_path); });
}

// Get volume info asynchronously
inline auto get_volume_info_sender(path p) {
    return stdexec::just(std::move(p)) | stdexec::then([](path file_path) { return get_volume_info(file_path); });
}

// List volumes asynchronously
inline auto list_volumes_sender() {
    return stdexec::just() | stdexec::then([]() { return list_volumes(); });
}

// Get space info asynchronously
inline auto space_sender(path p) {
    return stdexec::just(std::move(p)) | stdexec::then([](path file_path) { return frappe::space(file_path); });
}
...
inline auto lock_file_sender(path p, lock_options options = {}) {
    return stdexec::just(std::move(p), std::move(options)) |
           stdexec::then([](path file_path, lock_options opts) { return lock_file(file_path, opts); });
}
...
inline auto try_lock_file_sender(path p, lock_type type = lock_type::exclusive) {
    return stdexec::just(std::move(p), type) |
           stdexec::then([](path file_path, lock_type t) { return try_lock_file(file_path, t); });
}

// Check if file is locked asynchronously
inline auto is_file_locked_sender(path p) {
    return stdexec::just(std::move(p)) | stdexec::then([](path file_path) { return is_file_locked(file_path); });
}
...
inline auto wait_for_change_sender(path p, std::chrono::milliseconds timeout = std::chrono::milliseconds{0}) {
    return stdexec::just(std::move(p), timeout) |
           stdexec::then([](path file_path, std::chrono::milliseconds t) { return wait_for_change(file_path, t); });
}

// Check if watch is supported asynchronously
inline auto is_watch_supported_sender(path p) {
    return stdexec::just(std::move(p)) | stdexec::then([](path file_path) { return is_watch_supported(file_path); });
}
