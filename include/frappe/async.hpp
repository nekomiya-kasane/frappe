...
namespace frappe {
    ...
    inline auto get_disk_info_sender(std::string device_path) {
        return stdexec::just(std::move(device_path)) |
               stdexec::then([](std::string path) { return get_disk_info(path); });
    }
    ...
    inline auto get_file_owner_sid_sender(path p) {
        return stdexec::just(std::move(p)) |
               stdexec::then([](path file_path) { return get_file_owner_sid(file_path); });
    }
    ...
    inline auto is_device_ready_sender(path p) {
        return stdexec::just(std::move(p)) |
               stdexec::then([](path device_path) { return is_device_ready(device_path); });
    }

    // Check if media is present asynchronously
    inline auto is_media_present_sender(path p) {
        return stdexec::just(std::move(p)) |
               stdexec::then([](path device_path) { return is_media_present(device_path); });
    }
    ...
    inline auto get_cloud_provider_sender(path p) {
        return stdexec::just(std::move(p)) |
               stdexec::then([](path file_path) { return get_cloud_provider(file_path); });
    }
    ...
    inline auto is_virtual_filesystem_sender(path p) {
        return stdexec::just(std::move(p)) |
               stdexec::then([](path file_path) { return is_virtual_filesystem(file_path); });
    }
    ...
    inline auto is_encrypted_mount_sender(path p) {
        return stdexec::just(std::move(p)) |
               stdexec::then([](path file_path) { return is_encrypted_mount(file_path); });
    }

    // Get virtual filesystem info asynchronously
    inline auto get_virtual_fs_info_sender(path p) {
        return stdexec::just(std::move(p)) |
               stdexec::then([](path file_path) { return get_virtual_fs_info(file_path); });
    }
    ...
    inline auto get_filesystem_type_sender(path p) {
        return stdexec::just(std::move(p)) |
               stdexec::then([](path file_path) { return get_filesystem_type(file_path); });
    }
    ...
    inline auto is_watch_supported_sender(path p) {
        return stdexec::just(std::move(p)) |
               stdexec::then([](path file_path) { return is_watch_supported(file_path); });
    }
