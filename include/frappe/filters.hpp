...
struct filter_adapter {
    ...
    template<std::ranges::input_range R>
    friend auto operator|(R&& r, const filter_adapter& f) {
        return f(std::forward<R>(r));
    }
};

template<typename Pred>
[[nodiscard]] inline filter_adapter<Pred> filter(Pred pred) {
...
template<typename P>
[[nodiscard]] inline auto operator!(filter_adapter<P> f) {
    return filter(not_predicate<P>{std::move(f.predicate)});
}

// Functional combinators (alternative syntax)
template<typename P1, typename P2>
[[nodiscard]] inline auto and_(filter_adapter<P1> f1, filter_adapter<P2> f2) {
    return f1 && f2;
}

template<typename P1, typename P2>
[[nodiscard]] inline auto or_(filter_adapter<P1> f1, filter_adapter<P2> f2) {
    return f1 || f2;
}

template<typename P>
[[nodiscard]] inline auto not_(filter_adapter<P> f) {
    return !f;
}
...
[[nodiscard]] inline auto size_range(std::uintmax_t min_size, std::uintmax_t max_size) {
    return filter([min_size, max_size](const file_entry& e) { 
        return e.size >= min_size && e.size <= max_size; 
    });
}

inline const auto empty_files = filter([](const file_entry& e) { 
    return e.type == file_type::regular && e.size == 0; 
});
...
[[nodiscard]] inline auto mtime_range(file_time_type start, file_time_type end) {
    return filter([start, end](const file_entry& e) { 
        return e.mtime >= start && e.mtime <= end; 
    });
}

template<typename Duration>
[[nodiscard]] inline auto modified_within(Duration d) {
    auto threshold = file_time_type::clock::now() - d;
    return filter([threshold](const file_entry& e) { return e.mtime > threshold; });
}

template<typename Duration>
[[nodiscard]] inline auto accessed_within(Duration d) {
    auto threshold = file_time_type::clock::now() - d;
    return filter([threshold](const file_entry& e) { return e.atime > threshold; });
}

template<typename Duration>
[[nodiscard]] inline auto created_within(Duration d) {
    auto threshold = file_time_type::clock::now() - d;
...
inline const auto readable = filter([](const file_entry& e) { 
    return (e.permissions & perms::owner_read) != perms::none; 
});

inline const auto writable = filter([](const file_entry& e) { 
    return (e.permissions & perms::owner_write) != perms::none; 
});

inline const auto executable = filter([](const file_entry& e) { 
...
[[nodiscard]] inline auto extension_is(std::string_view ext) {
    ...
    return filter([ext_str = std::move(ext_str)](const file_entry& e) { 
        return e.extension == ext_str; 
    });
}
...
[[nodiscard]] inline auto name_contains(std::string_view substr) {
    ...
    return filter([s = std::move(s)](const file_entry& e) { 
        return e.name.find(s) != std::string::npos; 
    });
}
...
[[nodiscard]] inline auto path_contains(std::string_view substr) {
    std::string s(substr);
    return filter([s = std::move(s)](const file_entry& e) { 
        return e.file_path.string().find(s) != std::string::npos; 
    });
}
...
[[nodiscard]] inline auto name_regex(std::string_view pattern) {
    ...
    return filter([re](const file_entry& e) { 
        return re->match(e.name); 
    });
}
...
[[nodiscard]] inline auto path_regex(std::string_view pattern) {
    auto re = std::make_shared<compiled_regex>(pattern);
    return filter([re](const file_entry& e) { 
        return re->match(e.file_path.string()); 
    });
}
...
inline const auto has_multiple_links = filter([](const file_entry& e) { 
    return e.hard_link_count > 1; 
});
...
[[nodiscard]] inline auto under_path(const path& p) {
    ...
    return filter([prefix = std::move(prefix)](const file_entry& e) {
        return e.file_path.string().starts_with(prefix);
    });
}
...
[[nodiscard]] inline auto same_inode_as(const path& p) {
    ...
    return filter([ref_inode, valid](const file_entry& e) { 
        return valid && e.inode == ref_inode; 
    });
}
