...
// file_type formatter
template<>
struct std::formatter<std::filesystem::file_type> {
    bool use_char = false;
...
// perms formatter
template<>
struct std::formatter<std::filesystem::perms> {
...
template<>
struct std::formatter<frappe::filesystem_type> {
    constexpr auto parse(std::format_parse_context& ctx) {
        auto it = ctx.begin();
        if (it != ctx.end() && *it != '}') {
...
// file_id formatter
template<>
struct std::formatter<frappe::file_id> {
    constexpr auto parse(std::format_parse_context& ctx) {
        auto it = ctx.begin();
        if (it != ctx.end() && *it != '}') {
...
// link_type formatter
template<>
struct std::formatter<frappe::link_type> {
    constexpr auto parse(std::format_parse_context& ctx) {
        auto it = ctx.begin();
        if (it != ctx.end() && *it != '}') {
    ...
    auto format(frappe::link_type lt, std::format_context& ctx) const {
        switch (lt) {
            case frappe::link_type::none: return std::format_to(ctx.out(), "none");
            case frappe::link_type::symlink: return std::format_to(ctx.out(), "symlink");
            case frappe::link_type::hardlink: return std::format_to(ctx.out(), "hardlink");
...
// link_info formatter
template<>
struct std::formatter<frappe::link_info> {
    constexpr auto parse(std::format_parse_context& ctx) {
        auto it = ctx.begin();
        if (it != ctx.end() && *it != '}') {
...
template<>
struct std::formatter<frappe::owner_info> {
    ...
    auto format(const frappe::owner_info& info, std::format_context& ctx) const {
        switch (mode) {
            case 'u': return std::format_to(ctx.out(), "{}", info.user_name);
            case 'g': return std::format_to(ctx.out(), "{}", info.group_name);
            default: return std::format_to(ctx.out(), "{}:{}", info.user_name, info.group_name);
        }
    }
};
...
template<>
struct std::formatter<std::filesystem::space_info> {
    bool human = false;
    ...
    auto format(const std::filesystem::space_info& info, std::format_context& ctx) const {
        if (human) {
            return std::format_to(ctx.out(), "{}/{} ({})", 
                frappe::format_size_human(info.available),
                frappe::format_size_human(info.capacity),
                frappe::format_size_human(info.free));
        }
        return std::format_to(ctx.out(), "{}/{} ({} free)", 
            info.available, info.capacity, info.free);
    }
};
...
template<>
struct std::formatter<frappe::file_entry> {
    std::string fields;
    ...
    constexpr auto parse(std::format_parse_context& ctx) {
        ...
        if (it != ctx.end() && *it == ':') {
            ++it;
            while (it != ctx.end() && *it != '}') {
                if (*it == 'h') human_size = true;
                else if (*it == 'o') octal_perms = true;
                else if (*it == 'r') relative_path = true;
                else if (*it == '%') {
            ...
            switch (field) {
                ...
                case 'P':  // permissions
                    if (octal_perms) {
                        ...
                        out = std::format_to(out, "{}{}", 
                            frappe::format_file_type_char(e.type),
