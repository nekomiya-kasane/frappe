...
find_options &find_options::path_regex(std::string_view pattern) {
    auto re = std::make_shared<compiled_regex>(pattern);
    _conditions.push_back({[re](const std::filesystem::directory_entry &e) { return re->match(e.path().string()); }});
    return *this;
}
...
find_options &find_options::extension(std::string_view ext) {
    ...
    _conditions.push_back(
        {[extension_str](const std::filesystem::directory_entry &e) { return e.path().extension() == extension_str; }});
    return *this;
}
