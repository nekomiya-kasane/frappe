...
find_options& find_options::name(std::string_view pattern) {
    ...
    _conditions.push_back({[pat](const std::filesystem::directory_entry& e) {
        return fnmatch(e.path().filename().string(), pat);
    }});
    return *this;
}
...
find_options& find_options::regex(std::string_view pattern) {
    ...
    _conditions.push_back({[re](const std::filesystem::directory_entry& e) {
        return re->match(e.path().filename().string());
    }});
    return *this;
}
...
public:
    impl(const path& search_path, const find_options& options)
        : _options(options) {
