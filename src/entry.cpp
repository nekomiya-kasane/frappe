...
char file_type_indicator(file_type ft) noexcept {
    switch (ft) {
        case file_type::directory: return '/';
        case file_type::symlink: return '@';
        case file_type::fifo: return '|';
        case file_type::socket: return '=';
        case file_type::block: return '#';
        case file_type::character: return '%';
        default: return ' ';
    }
}
...
char file_type_indicator(const file_entry& e) noexcept {
    ...
    if (e.type == file_type::regular) {
        // Check if executable
        if ((e.permissions & perms::owner_exec) != perms::none ||
            (e.permissions & perms::group_exec) != perms::none ||
