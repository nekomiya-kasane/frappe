...
public:
    impl(std::string_view pattern, regex_options opts)
        : _pattern(pattern) {
        ...
        if (!_code) {
            ...
            throw regex_error(
                std::string("PCRE2 compilation failed at offset ") + 
...
std::optional<std::string> regex_replace(std::string_view text, std::string_view pattern, std::string_view replacement) {
    try {
        compiled_regex re(pattern);
...
std::optional<std::string> regex_replace(std::string_view text, const compiled_regex& re, std::string_view replacement) {
    return re.replace(text, replacement);
}
