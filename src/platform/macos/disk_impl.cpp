...
result<virtual_disk_info> get_virtual_disk_info_impl(const path& p) noexcept {
    ...
    if (ext == ".dmg") info.type = virtual_disk_type::dmg;
    else if (ext == ".iso") info.type = virtual_disk_type::iso;
    else if (ext == ".vhd") info.type = virtual_disk_type::vhd;
    ...
    else if (ext == ".sparseimage" || ext == ".sparsebundle") info.type = virtual_disk_type::sparse;
