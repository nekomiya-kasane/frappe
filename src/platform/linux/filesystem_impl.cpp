...
result<std::vector<volume_info>> list_volumes_impl() noexcept {
    ...
    for (const auto& entry : entries) {
        ...
        if (entry.fs_type == "proc" || entry.fs_type == "sysfs" || 
            entry.fs_type == "devpts" || entry.fs_type == "cgroup" ||
            entry.fs_type == "cgroup2" || entry.fs_type == "securityfs" ||
            entry.fs_type == "debugfs" || entry.fs_type == "configfs" ||
            entry.fs_type == "fusectl" || entry.fs_type == "mqueue" ||
            entry.fs_type == "hugetlbfs" || entry.fs_type == "pstore" ||
...
result<std::vector<network_share_info>> list_network_shares_impl() noexcept {
    ...
    auto entries = get_mount_entries();
    for (const auto& entry : entries) {
        if (entry.fs_type == "nfs" || entry.fs_type == "nfs4" ||
            entry.fs_type == "cifs" || entry.fs_type == "smbfs") {
...
result<virtual_fs_info> get_virtual_fs_info_impl(const path& p) noexcept {
    ...
    const std::string& fs = mount->fs_type;
    if (fs.find("fuse") == 0) info.type = virtual_fs_type::fuse;
    else if (fs == "tmpfs") info.type = virtual_fs_type::tmpfs;
    else if (fs == "ramfs") info.type = virtual_fs_type::ramfs;
    else if (fs == "overlay") info.type = virtual_fs_type::overlay;
    else if (fs == "proc") info.type = virtual_fs_type::procfs;
    else if (fs == "sysfs") info.type = virtual_fs_type::sysfs;
    
    if (fs == "fuse.sshfs") info.type = virtual_fs_type::sshfs;
...
result<std::vector<virtual_fs_info>> list_virtual_mounts_impl() noexcept {
    ...
    for (const auto& entry : entries) {
        ...
        if (fs.find("fuse") == 0 || fs == "tmpfs" || fs == "ramfs" || 
            fs == "overlay" || fs == "proc" || fs == "sysfs" || fs == "devtmpfs") {
...
namespace {
    ...
    void parse_mount_options(const std::string& opts_str, mount_point_info& info) {
        ...
        while (std::getline(iss, opt, ',')) {
            ...
            if (opt == "ro") info.is_readonly = true;
            else if (opt == "noexec") info.is_noexec = true;
            else if (opt == "nosuid") info.is_nosuid = true;
            else if (opt == "nodev") info.is_nodev = true;
            else if (opt == "bind") info.is_bind = true;
            else if (opt == "loop") info.is_loop = true;
        }
    }
}
...
result<std::vector<mount_point_info>> list_all_mounts_impl() noexcept {
    ...
    for (const auto& entry : entries) {
        ...
        for (const auto& other : entries) {
            ...
            if (this_mp.starts_with(other_mp) && 
                (other_mp == "/" || this_mp[other_mp.length()] == '/')) {
...
result<mount_point_info> get_mount_point_info_impl(const path& p) noexcept {
    ...
    for (const auto& m : *all_mounts) {
        ...
        if (path_str.starts_with(mount_str)) {
            if (mount_str == "/" || path_str.length() == mount_str.length() || 
                path_str[mount_str.length()] == '/') {
