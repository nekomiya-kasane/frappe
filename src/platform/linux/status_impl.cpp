...
result<file_time_type> creation_time_impl(const path& p) noexcept {
    ...
    auto duration = std::chrono::seconds(st.st_birthtime) + 
                    std::chrono::nanoseconds(st.st_birthtimensec);
...
#else
    return file_time_type(std::chrono::duration_cast<file_time_type::duration>(
        std::chrono::seconds(st.st_ctime)
    ));
#endif
}
...
#ifdef ___linux_
    auto duration = std::chrono::seconds(st.st_atime) + 
                    std::chrono::nanoseconds(st.st_atim.tv_nsec);
    return file_time_type(std::chrono::duration_cast<file_time_type::duration>(duration));
#else
    return file_time_type(std::chrono::duration_cast<file_time_type::duration>(
