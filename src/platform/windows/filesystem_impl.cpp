...
result<filesystem_features> get_filesystem_features_impl(const path& p) noexcept {
    ...
    if (!GetVolumeInformationW(volume_path.c_str(), nullptr, 0, &serial, &max_component, &flags, fs_name, MAX_PATH)) {
        return std::unexpected(make_error(static_cast<int>(GetLastError()), std::system_category()));
    }
    ...
    if (GetDiskFreeSpaceW(volume_path.c_str(), &sectors_per_cluster, &bytes_per_sector, &free_clusters, &total_clusters)) {
...
result<volume_info> get_volume_info_impl(const path& p) noexcept {
    ...
    if (GetVolumeInformationW(volume_path.c_str(), volume_name, MAX_PATH, &serial, &max_component, &flags, fs_name, MAX_PATH)) {
...
result<std::vector<removable_device_info>> list_removable_devices_impl() noexcept {
    ...
    for (int i = 0; i < 26; ++i) {
        ...
        if (drive_type == DRIVE_REMOVABLE || drive_type == DRIVE_CDROM) {
            ...
            if (GetVolumeInformationW(drive_path, volume_name, MAX_PATH, &serial, nullptr, nullptr, fs_name, MAX_PATH)) {
                ...
                if (fs_str == "NTFS") info.fs_type = filesystem_type::ntfs;
                else if (fs_str == "FAT32") info.fs_type = filesystem_type::fat32;
                else if (fs_str == "exFAT") info.fs_type = filesystem_type::exfat;
                else if (fs_str == "FAT") info.fs_type = filesystem_type::fat;
            }
...
result<virtual_fs_info> get_virtual_fs_info_impl(const path& p) noexcept {
    ...
    if (GetVolumeInformationW(root.c_str(), nullptr, 0, nullptr, nullptr, nullptr, fs_name, MAX_PATH)) {
        std::wstring fs(fs_name);
        if (fs == L"WinFsp") info.type = virtual_fs_type::winfsp;
        else if (fs == L"Dokan") info.type = virtual_fs_type::dokan;
    }
...
namespace {
    storage_location_type get_storage_type_for_drive(UINT drive_type) {
        switch (drive_type) {
            case DRIVE_REMOVABLE: return storage_location_type::removable;
            case DRIVE_FIXED: return storage_location_type::local;
            case DRIVE_REMOTE: return storage_location_type::network_share;
            case DRIVE_CDROM: return storage_location_type::removable;
            case DRIVE_RAMDISK: return storage_location_type::ram;
            default: return storage_location_type::unknown;
        }
    }
            ...
            if (GetVolumePathNamesForVolumeNameW(volume_name, mount_paths, MAX_PATH * 4, &chars_returned)) {
                ...
                while (*current) {
                    ...
                    if (!found && mp.length() > 3) {  // Not a drive root
                        ...
                        wchar_t fs_name[MAX_PATH] = {0};
                        if (GetVolumeInformationW(volume_name, nullptr, 0, nullptr, nullptr, nullptr, fs_name, MAX_PATH)) {
                            info.fs_type = get_fs_type_from_name(fs_name);
                        }
