...
namespace frappe::detail {
    ...
    namespace {
        ...
        disk_bus_type get_bus_type(STORAGE_BUS_TYPE bus) {
            switch (bus) {
            case BusTypeAta: return disk_bus_type::ata;
            case BusTypeSata: return disk_bus_type::sata;
            case BusTypeScsi: return disk_bus_type::scsi;
            case BusTypeSas: return disk_bus_type::sas;
            case BusTypeUsb: return disk_bus_type::usb;
            case BusTypeNvme: return disk_bus_type::nvme;
            case BusTypeRAID: return disk_bus_type::raid;
            case BusTypeMmc: return disk_bus_type::mmc;
            case BusTypeVirtual: return disk_bus_type::virtual_bus;
            default: return disk_bus_type::unknown;
            }
        }
    }
    ...
    result<std::vector<disk_info>> list_disks_impl() noexcept {
        ...
        for (int i = 0; i < 32; ++i) {
            ...
            if (DeviceIoControl(hDisk, IOCTL_DISK_GET_DRIVE_GEOMETRY_EX, nullptr, 0,
                &geometry, sizeof(geometry), &bytes_returned, nullptr)) {
            ...
            if (DeviceIoControl(hDisk, IOCTL_STORAGE_QUERY_PROPERTY, &query, sizeof(query),
                buffer, sizeof(buffer), &bytes_returned, nullptr)) {
                ...
                if (info.bus_type == disk_bus_type::nvme) {
                    ...
                    if (DeviceIoControl(hDisk, IOCTL_STORAGE_QUERY_PROPERTY, &seek_query, sizeof(seek_query),
                        &seek_desc, sizeof(seek_desc), &bytes_returned, nullptr)) {
            ...
            if (DeviceIoControl(hDisk, IOCTL_DISK_GET_DRIVE_LAYOUT_EX, nullptr, 0,
                layout_buffer, sizeof(layout_buffer), &bytes_returned, nullptr)) {
    ...
    result<disk_info> get_disk_for_path_impl(const path& p) noexcept {
        ...
        HANDLE hVol = CreateFileW(vol_str.c_str(), 0, FILE_SHARE_READ | FILE_SHARE_WRITE,
        ...
        if (!DeviceIoControl(hVol, IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS, nullptr, 0,
            &extents, sizeof(extents), &bytes_returned, nullptr)) {
            CloseHandle(hVol);
            return std::unexpected(make_error(static_cast<int>(GetLastError()), std::system_category()));
        }
    ...
    result<partition_table_info> get_partition_table_impl(std::string_view device_path) noexcept {
        ...
        HANDLE hDisk = CreateFileW(wpath.c_str(), 0, FILE_SHARE_READ | FILE_SHARE_WRITE,
            nullptr, OPEN_EXISTING, 0, nullptr);
        ...
        for (DWORD i = 0; i < layout->PartitionCount; ++i) {
            ...
            if (layout->PartitionStyle == PARTITION_STYLE_GPT) {
                ...
                pinfo.is_primary = !part.Mbr.RecognizedPartition ||
                    (part.Mbr.PartitionType != 0x05 && part.Mbr.PartitionType != 0x0F);
    ...
    result<std::vector<mount_info>> list_mounts_impl() noexcept {
        ...
        for (int i = 0; i < 26; ++i) {
            ...
            if (GetVolumeInformationW(drive_path.c_str(), volume_name, MAX_PATH,
    ...
    result<virtual_disk_info> get_virtual_disk_info_impl(const path& p) noexcept {
        ...
        else if (ext == ".qcow" || ext == ".qcow2") info.type = virtual_disk_type::qcow2;
