...
namespace frappe::detail {
    ...
    namespace {
        ...
        partition_type get_partition_type_from_guid(const GUID &guid) {
            ...
            static const GUID EFI_SYSTEM = {
                0xC12A7328, 0xF81F, 0x11D2, {0xBA, 0x4B, 0x00, 0xA0, 0xC9, 0x3E, 0xC9, 0x3B}};
            // Microsoft Basic Data
            static const GUID MS_BASIC_DATA = {
                0xEBD0A0A2, 0xB9E5, 0x4433, {0x87, 0xC0, 0x68, 0xB6, 0xB7, 0x26, 0x99, 0xC7}};
            // Microsoft Reserved
            static const GUID MS_RESERVED = {
                0xE3C9E316, 0x0B5C, 0x4DB8, {0x81, 0x7D, 0xF9, 0x2D, 0xF0, 0x02, 0x15, 0xAE}};
            // Microsoft Recovery
            static const GUID MS_RECOVERY = {
        ...
        std::string guid_to_string(const GUID &guid) {
            char buffer[64];
            snprintf(buffer, sizeof(buffer), "%08lX-%04X-%04X-%02X%02X-%02X%02X%02X%02X%02X%02X", guid.Data1,
    ...
    result<std::vector<mount_info>> list_mounts_impl() noexcept {
        ...
        for (int i = 0; i < 26; ++i) {
            ...
            if (GetVolumeInformationW(drive_path.c_str(), volume_name, MAX_PATH, &serial, &max_component, &flags,
                                      fs_name, MAX_PATH)) {
    ...
    result<smart_info> get_smart_info_impl(std::string_view device_path) noexcept {
        ...
        std::wstring wpath(device_path.begin(), device_path.end());
        HANDLE hDisk = CreateFileW(wpath.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE,
                                   nullptr, OPEN_EXISTING, 0, nullptr);

        if (hDisk == INVALID_HANDLE_VALUE) {
            // Try read-only access
            hDisk = CreateFileW(wpath.c_str(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING,
                                0, nullptr);
            if (hDisk == INVALID_HANDLE_VALUE) {
    ...
    result<nvme_info> get_nvme_info_impl(std::string_view device_path) noexcept {
        ...
        std::wstring wpath(device_path.begin(), device_path.end());
        HANDLE hDisk = CreateFileW(wpath.c_str(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr,
