    ...
    void file_watcher::impl::watch_loop() {
        ...
        // Open directory handles
        {
            ...
            for (const auto &p : _watched_paths) {
                HANDLE hDir =
                    CreateFileW(p.c_str(), FILE_LIST_DIRECTORY, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
        ...
        for (size_t i = 0; i < handles.size(); ++i) {
            ReadDirectoryChangesW(handles[i], buffers[i].data(), static_cast<DWORD>(buffers[i].size()),
                                  _options.recursive ? TRUE : FALSE, filter, nullptr, &overlappeds[i], nullptr);
        }
        ...
        while (_running) {
            ...
            if (GetOverlappedResult(handles[idx], &overlappeds[idx], &bytes_returned, FALSE) && bytes_returned > 0) {
                process_notify_info(reinterpret_cast<FILE_NOTIFY_INFORMATION *>(buffers[idx].data()),
                                    handle_paths[idx]);
            }
