...
namespace {
    ...
    std::vector<BYTE> get_sid_from_name(std::string_view name) {
        ...
        if (LookupAccountNameW(nullptr, wname.c_str(), sid_buffer.data(), &sid_size, 
                               domain_buffer.data(), &domain_size, &sid_type)) {
            return sid_buffer;
        }
        return {};
    }
    ...
    for (const auto& entry : acl.entries) {
        ...
        if (!entry.qualifier.empty()) {
            ...
            if (sid_size > 0) {
                ...
                if (!LookupAccountNameW(nullptr, wname.c_str(), sid, &sid_size, domain.data(), &domain_size, &sid_type)) {
                    LocalFree(sid);
                    sid = nullptr;
                }
            }
...
namespace {
    sid_type convert_sid_name_use(SID_NAME_USE use) {
        switch (use) {
            case SidTypeUser: return sid_type::user;
            case SidTypeGroup: return sid_type::group;
            case SidTypeDomain: return sid_type::domain;
            case SidTypeAlias: return sid_type::alias;
            case SidTypeWellKnownGroup: return sid_type::well_known_group;
            case SidTypeDeletedAccount: return sid_type::deleted_account;
            case SidTypeInvalid: return sid_type::invalid;
            case SidTypeComputer: return sid_type::computer;
            case SidTypeLabel: return sid_type::label;
            default: return sid_type::unknown;
        }
    }
}
