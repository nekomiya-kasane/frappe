...
std::string acl_to_string(const acl_info& acl) noexcept {
    ...
    for (const auto& entry : acl.entries) {
        switch (entry.tag) {
            case acl_tag::user_obj: oss << "user::"; break;
            case acl_tag::user: oss << "user:" << entry.qualifier << ":"; break;
            case acl_tag::group_obj: oss << "group::"; break;
            case acl_tag::group: oss << "group:" << entry.qualifier << ":"; break;
            case acl_tag::mask: oss << "mask::"; break;
            case acl_tag::other: oss << "other::"; break;
        }
        oss << (entry.read ? 'r' : '-');
        oss << (entry.write ? 'w' : '-');
        oss << (entry.execute ? 'x' : '-');
...
void_result remove_acl_entry(const path& p, const acl_entry& entry) noexcept {
    ...
    entries.erase(
        std::remove_if(entries.begin(), entries.end(), [&](const acl_entry& e) {
            return e.tag == entry.tag && e.qualifier == entry.qualifier;
        }),
        entries.end()
    );
    return set_acl(p, *acl);
...
void_result remove_win_attribute(const path& p, win_attributes attr) noexcept {
    auto current = get_win_attributes(p);
    if (!current) return std::unexpected(current.error());
    auto new_attrs = static_cast<win_attributes>(
        static_cast<std::uint32_t>(*current) & ~static_cast<std::uint32_t>(attr)
    );
...
std::string_view sid_type_name(sid_type type) noexcept {
    switch (type) {
        case sid_type::unknown: return "unknown";
        case sid_type::user: return "user";
        case sid_type::group: return "group";
        case sid_type::domain: return "domain";
        case sid_type::alias: return "alias";
        case sid_type::well_known_group: return "well_known_group";
        case sid_type::deleted_account: return "deleted_account";
        case sid_type::invalid: return "invalid";
        case sid_type::computer: return "computer";
        case sid_type::label: return "label";
