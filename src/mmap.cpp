...
mapped_file::mapped_file(std::unique_ptr<impl> impl) noexcept 
    : _impl(std::move(impl)) {}
...
result<mapped_file> mapped_file::open(
    const path& p,
    std::size_t offset,
    std::size_t length,
    map_mode mode
    ...
    pimpl->mapping_handle = CreateFileMappingW(
        pimpl->file_handle,
        nullptr,
        protect,
        0,
    ...
    pimpl->data = static_cast<std::byte*>(MapViewOfFile(
...
result<mapped_file> mapped_file::create(
    const path& p,
    std::size_t size,
    map_mode mode
) noexcept {
...
void_result mapped_file::advise(std::size_t offset, std::size_t length, map_advice advice) noexcept {
    ...
    int adv = MADV_NORMAL;
    switch (advice) {
        case map_advice::normal: adv = MADV_NORMAL; break;
        case map_advice::sequential: adv = MADV_SEQUENTIAL; break;
        case map_advice::random: adv = MADV_RANDOM; break;
        case map_advice::willneed: adv = MADV_WILLNEED; break;
        case map_advice::dontneed: adv = MADV_DONTNEED; break;
    }
...
void_result mapped_file::resize(std::size_t new_size) noexcept {
    ...
    _impl->mapping_handle = CreateFileMappingW(
        _impl->file_handle,
        nullptr,
        PAGE_READWRITE,
        li.HighPart,
    ...
    _impl->data = static_cast<std::byte*>(MapViewOfFile(
...
mapped_view::mapped_view(const mapped_file& file) noexcept
    : _data(file.data()), _size(file.size()) {}
...
mapped_view::mapped_view(const std::byte* data, std::size_t size) noexcept
...
mapped_file_chunks::mapped_file_chunks(const path& p, std::size_t chunk_size)
    : _path(p), _chunk_size(chunk_size) {
    std::error_code ec;
