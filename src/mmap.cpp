...
result<mapped_file> mapped_file::open(const path &p, std::size_t offset, std::size_t length, map_mode mode) noexcept {
    ...
    pimpl->file_handle = CreateFileW(p.c_str(), access, share, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
...
result<mapped_file> mapped_file::create(const path &p, std::size_t size, map_mode mode) noexcept {
    ...
    pimpl->mapping_handle = CreateFileMappingW(pimpl->file_handle, nullptr, protect, li.HighPart, li.LowPart, nullptr);
...
void_result mapped_file::resize(std::size_t new_size) noexcept {
    ...
    _impl->data = static_cast<std::byte *>(MapViewOfFile(_impl->mapping_handle, FILE_MAP_ALL_ACCESS, 0, 0, new_size));
