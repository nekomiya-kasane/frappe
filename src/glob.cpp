...
void glob_impl(const path& base_path, std::string_view pattern, 
               std::vector<path>& results, bool recursive = false) {
...
public:
    impl(const path& base_path, std::string_view pattern)
        : _results(glob(base_path, pattern)), _index(0) {}
...
glob_range::glob_range(std::string_view pattern)
    : _base_path("."), _pattern(pattern) {
    auto [base, pat] = split_pattern(pattern);
    _base_path = base;
    _pattern = pat;
}
