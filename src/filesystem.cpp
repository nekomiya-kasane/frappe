...
result<std::vector<removable_device_info>> list_usb_devices() noexcept {
    ...
    for (const auto &d : *all) {
        if (d.device_type == removable_device_type::usb_drive || d.device_type == removable_device_type::usb_stick ||
...
result<std::vector<mount_point_info>> list_network_mounts() noexcept {
    ...
    for (const auto &m : *all_mounts) {
        if (m.storage_type == storage_location_type::network_share ||
            m.storage_type == storage_location_type::network_drive || m.storage_type == storage_location_type::nfs) {
            network.push_back(m);
        }
    }
    return network;
