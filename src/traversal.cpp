...
cycle_detector::cycle_detector(cycle_detection strategy)
    : _strategy(strategy) {}
...
struct directory_traverser::impl {
    ...
    impl(const path& r, const traversal_options& o)
        : root(r), opts(o), detector(o.cycle_strategy) {
...
directory_traverser::iterator::iterator(std::shared_ptr<impl> impl)
    : _impl(std::move(impl)) {}
...
std::vector<file_entry> traverse_parallel(
    const path& root,
    const traversal_options& opts,
...
struct safe_recursive_iterator::impl {
    ...
    impl(const path& root, const traversal_options& o)
        : opts(o), detector(o.cycle_strategy) {
        std::error_code ec;
        auto dir_opts = std::filesystem::directory_options::none;
        if (opts.skip_permission_denied) {
            dir_opts |= std::filesystem::directory_options::skip_permission_denied;
