...
void demo_basic_listing() {
    ...
    std::cout << std::left << std::setw(40) << "Name" << std::right << std::setw(10) << "Size" << std::setw(20)
              << "Modified" << "\n";
    std::cout << std::string(70, '-') << "\n";

    for (const auto &e : entries | entry::sort_by_name | entry::take(10)) {
        std::cout << std::left << std::setw(40) << e.name << std::right << std::setw(10) << format_size_human(e.size)
...
void demo_aggregation() {
    ...
    if (largest_file) {
        std::cout << "Largest file: " << largest_file->name << " (" << format_size_human(largest_file->size) << ")\n";
    }

    // Find newest file
    auto newest_file = entries | files_only | newest;
    if (newest_file) {
        std::cout << "Newest file: " << newest_file->name << " (" << format_time_ls(newest_file->mtime) << ")\n";
    }
...
void demo_formatting() {
    ...
    std::cout << "\nls -l style:\n";
    for (const auto &e : entries) {
        std::cout << format_file_type_char(e.type) << format_permissions(e.permissions) << " " << std::setw(3)
                  << e.hard_link_count << " " << std::setw(8) << e.owner << " " << std::setw(8) << e.group << " "
                  << std::setw(8) << e.size << " " << format_time_ls(e.mtime) << " " << e.name;
        if (e.is_symlink) {
...
void demo_complex_pipeline() {
    ...
    auto results = list_entries_recursive(dir, {.include_hidden = false}) | files_only |
                   extension_in({".cpp", ".hpp", ".h", ".c"}) | modified_this_week() | entry::sort_by_size_desc |
                   entry::take(5);

    std::cout << "Top 5 largest C++ files modified this week:\n";
    std::cout << std::left << std::setw(50) << "Path" << std::right << std::setw(10) << "Size" << std::setw(20)
              << "Modified" << "\n";
    std::cout << std::string(80, '-') << "\n";

    for (const auto &e : results) {
        std::cout << std::left << std::setw(50) << e.file_path.filename().string() << std::right << std::setw(10)
                  << format_size_human(e.size) << std::setw(20) << format_time_ls(e.mtime) << "\n";
    }
