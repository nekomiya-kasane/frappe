...
void demo_recursive_search() {
    ...
    auto options = frappe::find_options{}
        .is_file()
        .extension("hpp")
        .max_depth(3);
    
    // Pipeline: find_lazy -> sort by name -> take first 10
    auto header_files = frappe::find_lazy(search_dir, options)
        | frappe::sort_by_name
        | frappe::take(10);
...
void demo_complex_pipeline() {
    ...
    auto recent_sources = std::filesystem::recursive_directory_iterator(search_dir,
...
void demo_with_find_options() {
    ...
    auto files = frappe::find(search_dir, options)
        | frappe::sort_by_mtime_desc
        | frappe::take(5);
