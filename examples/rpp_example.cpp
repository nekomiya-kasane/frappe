...
int main() {
    ...
    auto high_paid_engineers =
        people | rpp::filter([](const Person &p) { return p.department == "Engineering" && p.salary > 70000; }) |
        rpp::to_vector;
    ...
    auto age_range = people | rpp::filters::between(&Person::age, 25, 32) | rpp::to_vector;
    ...
    auto sorted_by_salary = people | rpp::sort_by(&Person::salary).desc();
    ...
    std::set<Person, decltype(rpp::compare_by(&Person::name))> people_set(rpp::compare_by(&Person::name));
    ...
    auto multi_sorted =
        people | rpp::multi_sort(rpp::sort_by(&Person::department), rpp::sort_by(&Person::salary).desc());
    ...
    if (max_salary_person) {
        std::cout << "   Highest paid: " << max_salary_person->name << " ($" << max_salary_person->salary << ")\n";
    }
    ...
    auto name_salary = people | rpp::select(&Person::name, &Person::salary) | rpp::to_vector;
    ...
    auto unique_depts = people | rpp::distinct(&Person::department);
    ...
    auto joined = people | rpp::join(departments, &Person::department, &Department::name,
    ...
    auto with_row_nums =
        people |
        rpp::window(&Person::department, &Person::salary).select(rpp::window_fns::row_number, rpp::window_fns::rank);

    for (const auto &[person, window_vals] : with_row_nums) {
        auto [row_num, rank] = window_vals;
        std::cout << "   " << person.department << " - " << person.name << " (row=" << row_num << ", rank=" << rank
                  << ")\n";
    }
    ...
    auto top_engineers = people | rpp::filter([](const Person &p) { return p.department == "Engineering"; }) |
                         rpp::sort_by(&Person::salary).desc() | ::ranges::views::take(3);
    ...
    auto top_sales = people | rpp::filter([](const Person &p) { return p.department == "Sales"; }) |
                     rpp::sort_by(&Person::salary).desc() | ::ranges::views::take(2);
    ...
    auto doubled = people |
                   rpp::map([](const Person &p) { return Person{p.name, p.age, p.department, p.salary * 2}; }) |
                   rpp::take(3) | rpp::to_vector;
